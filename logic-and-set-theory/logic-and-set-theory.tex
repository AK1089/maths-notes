...
\begin{prf}
    ...
    Now, we formalise this. We start with three observations:
    \begin{enumerate}
    	\item If $S$ is a consistent theory and $p$ is a sentence, then either $S \cup p$ or $S \cup \set{\lnot p}$. If not, by the Deduction Theorem (\ref{first-order-deduction-theorem}), $S \vdash \lnot p$ and $S \vdash \lnot \lnot p$, but then $S \vdash \bot$, a contradiction.
